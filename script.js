...
const VOICEFLOW_API_KEY = 'VF.DM.68560bad29ae94126a7079ad.w4jItkj61Fk1vLQz';
const API_URL = 'https://general-runtime.voiceflow.com/state/user/USER_ID/interact';
...
async function initChat() {
    try {
        ...
        if (response && Array.isArray(response) && response.length > 0) {
            // Process the response but filter out any empty or system messages
            ...
            if (validResponses.length > 0) {
                // Small delay to ensure the overlay is hidden before showing messages
                setTimeout(() => {
...
async function sendToVoiceflow(request) {
    const url = API_URL.replace('USER_ID', userId);
